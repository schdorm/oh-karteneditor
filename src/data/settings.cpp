...
Settings::Settings()
{
  ...
  if(!dir.cd(".OpenHanse"))
  {
    if(dir.mkdir(".OpenHanse"))
    {
      ...
      if(!dir.cd("mapeditor"))
      {
	if(!dir.mkdir("mapeditor"))
	{
	  qFatal("Could not create OpenHanse-Mapeditor settings folder");
	}
      }
    }
    else
  ...
  m_ObjectFunctionList.insert(MapObject::Land_breake, land_breakelabel);
  m_ObjectFunctionList.insert(MapObject::Land_damage, land_damagelabel);
  m_ObjectFunctionList.insert(MapObject::Mapdecoration, mapdecorationlabel);
  
  QString mt_sea = tr("See/Meer"),
  mt_coast = tr("Kueste"),
  mt_land = tr("Land")/*, mt_city = tr("(Stadt)")*/;
...
const QHash<int, QString> &Settings::ObjectFunctionsRef() const
{
  return m_ObjectFunctionList;
}
...
const QHash<int, QString> &Settings::MapTypesRef() const
{
    return m_MapTypeList;
}


QString Settings::ohconfPath() const
{
  return m_OHConfPath;
}
